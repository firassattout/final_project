  ...
  <head>
    ...
    <style>
     ...
     height: 60px;
        ...
        background: #fff;
        transition: transform 0.3s ease;
            margin:  ${position == "top" ? "0 0 0 0" : "15px 0 0 0"};
      }

      .ad-container.hidden {
        transform: ${
          position == "top" ? "translateY(-110%)" : "translateY(110%)"
        };
      }
  ...
  <body>
    ...
    <script nonce="${nonce}">
      ...
      document.getElementById("ad-link").addEventListener("click", async () => {
        ...
        window.ReactNativeWebView.postMessage("Fetch error: " + error.message);
        }
      });
...
function generateRewardedEmbedCode(ad, mediaUrl, userId, nonce, duration = 15) {
  if (ad.ad.platform === "web")
    return `
    <!DOCTYPE html>
    <html>
